...
def generate_iso_name():
    ...
    return f"ubuntu-autoinstall-{timestamp}-{random_str}.iso"

# Generate the ISO filename
ISO_FILENAME = generate_iso_name()

# Remote working directories and files
REMOTE_WORKING_DIR = "/root/iso_mod"
REMOTE_EXTRACT_DIR = f"{REMOTE_WORKING_DIR}/extract"
REMOTE_ISO_FILE = "/var/www/html/ubuntu-24.04.2-live-server-amd64.iso"
...
def modify_grub():
    ...
    # Define the autoinstall menu entry
...
def create_cloud_init():
    ...
    # Create user-data file with the autoinstall configuration
...
def main():
    ...
    print("="*80)
    print("AUTOMATED UBUNTU ISO CREATION AND IDRAC DEPLOYMENT")
    print("="*80)
    
    print(f"\n📀 Using randomly generated ISO filename: {ISO_FILENAME}")
    
    # Step 1: Create the custom ISO
    print("\n📀 STEP 1: Creating custom Ubuntu ISO on Proxmox server...\n")
    if create_custom_iso():
        ...
        if mount_iso_on_idrac():
            print("\n✅ SUCCESS: The Ubuntu autoinstall process has been successfully initiated!")
        ...
        print("\n❌ ERROR: Failed to create the custom Ubuntu ISO.")
